            ...
            <div className='w-[95%] mx-auto pt-5'>
                <div className='w-full py-10 flex justify-between'>
                    <div className=' pl-1'>
                        ...
                        <p className='pl-1 poppins text-[13px] font-[500] text-[#696969b6]'>Total Products : {product_data?.products?.length}</p>
                    </div>
                    <div>
                        <button className='px-4 py-[6px] shadow-md bg-[color:var(--secondary-color)] text-white rounded-[8px] active:scale-[0.95]'>Add Products</button>
                    </div>
                </div>
                ...
                <div className='w-full bg-white rounded-[15px] py-5 px-4 shadow-md'>
                    <div className='w-full  border-b flex justify-start items-center px-4 my-4'>
                        <span className=''><img src={search} className="w-[15px]" /></span><input type="text" className='w-full px-3 py-2 outline-none text-[14px]' placeholder='Filter Products' onChange={(e) => setSearchData(e.target.value)} />
                    </div>
                    <div className='w-full rounded-[15px] '>
                        ...
                        <div className='w-full  rounded-b-[15px] grid grid-cols-6 text-[13px] text-[#464646] max-h-[530px] overflow-y-scroll pl-1'>
                            {
                                product_data?.products?.filter((filterValue) => {
                                    if (searchData === '') {
                                        return filterValue
                                    } else if (filterValue?.product?.toLowerCase()?.includes(searchData?.toLowerCase()) || filterValue?.product_id?.toLowerCase()?.includes(searchData?.toLowerCase()) || filterValue?.seller?.toLowerCase()?.includes(searchData?.toLowerCase()) || filterValue?.remarks?.toLowerCase()?.includes(searchData?.toLowerCase())) {
                                        return filterValue
                                    }
                                }).map((data, i) => (
